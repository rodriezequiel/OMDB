...
export const registerUser = createAsyncThunk("registerUser", user => {
  return axios
    .post(`${process.env.API_URL}/api/auth/register`, user)
    .then(res => {
...
export const loginUser = createAsyncThunk("loginUser", user => {
  return axios
    .post(`${process.env.API_URL}/api/auth/login`, user)
    .then(res => res.data)
    .then(data => {
...
export const fetchUser = createAsyncThunk('fetchUser', () =>{
  return axios.get(`${process.env.API_URL}/api/auth/me`)
    .then(res => res.data)
    .then(data => {
      message.success(`Welcome Back ${data.name}!`);
      return data;
...
export const logoutUser = createAsyncThunk("logoutUser", () => {
  return axios.post(`${process.env.API_URL}/api/auth/logout`).then(res => console.log(res));
});
...
export const addFav = createAsyncThunk("addFav", (fav, ThunkApi) => {
  ...
  if (!user.favs.some(element => element.imdbID === fav.movie.imdbID)) {
    return axios
      .put(`${process.env.API_URL}/api/favourites`, fav)
      .then(res => res.data)
      .then(favs => favs)
...
export const getFav = createAsyncThunk("getFav", id => {
  return axios
    .get(`${process.env.API_URL}/api/favourites?userID=${id}`)
    .then(res => res.data)
    .then(favs => favs)
...
export const deleteFav = createAsyncThunk('deleteFav', (query) =>{
  return axios.delete(`${process.env.API_URL}/api/favourites?imdbID=${query.imdbID}&userId=${query.userId}`)
    .then(res=> res.data)
    .then(favs => favs)
