...
export const logoutUser = createAsyncThunk("logoutUser", () => {
  return axios.post("/api/auth/logout").then(res => console.log(res));
});
...
export const deleteFav = createAsyncThunk('deleteFav', (query) =>{
  return axios.delete(`/api/favourites?imdbID=${query.imdbID}&userId=${query.userId}`)
    .then(res=> res.data)
    .then(favs => favs)
